...
main() {
    echo ""
    echo "      ,ww"
    echo "     wWWWWWWW_)  flock installer"
    echo "     \`WWWWWW'    flock.dev"
    echo "      II  II"
    echo ""
    ...
    MANIFEST="$(curl -fsSL --retry 3 --connect-timeout 10 --max-time 20 "$MANIFEST_URL")" \
        || err "can't reach ${MANIFEST_URL}. Please try again later; flock.dev might be down. Who let the sheeps out? baaa."
...
need() {
    if ! command -v "$1" >/dev/null 2>&1; then
        err "requires '$1' — install it first, or download a binary manually from https://flock.dev/docs/install/"
    fi
}
