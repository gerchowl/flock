...
Flock 0.6.3 pushes on three of those boundaries.

### Agent restore after server restart

When a Flock server stops, normal pane processes stop with it. Native agent restore is the path for getting supported agent conversations back after that kind of restart.

With `[session] resume_agents_on_restore = true`, Flock can use official session references from supported integrations to relaunch Pi, Claude Code, Codex, OpenCode, and Hermes panes into their previous conversations.

[Read the session restore docs](/docs/session-state/#native-agent-session-restore).

### Pane screen history

Pane screen history solves the visual half of restart recovery. When enabled, Flock saves recent pane output and replays it into restored panes so the workspace does not come back as a set of empty terminals.
...
Live handoff is the update path for supported running sessions. Instead of stopping the old server and killing pane processes, Flock can ask the old server to transfer live PTY ownership to the updated server.

It is currently experimental and opt-in with `flock update --handoff` and `flock --remote <host> --handoff`. That keeps the default path conservative while the feature gets more real-world coverage.
...
Large Flock sessions need fast movement. The new session navigator opens with `prefix+g` and gives you a searchable workspace, tab, and pane tree.
...
Flock also gained better update and remote-bootstrap behavior for package-manager installs, more reliable event delivery under load, and fixes across agent detection, terminal colors, sound alerts, and integration lifecycle handling.

## Pane metadata reporting

The new `pane.report_metadata` socket method and `flock pane report-metadata` CLI command let integrations and local hooks customize what Flock shows for a pane.
...
This gives integrations a supported way to make panes more readable while keeping Flock's session and agent ownership rules intact.
...
[Open the 0.6.3 GitHub release](https://github.com/gerchowl/flock/releases/tag/v0.6.3).
