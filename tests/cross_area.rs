...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/flock-cross-area-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket_path: &Path) -> SpawnedFlock {
...
fn cross_area_detach_and_reattach_preserves_state() {
    ...
    client_handshake(&mut client_a, support::PROTOCOL_VERSION, 100, 30);
    assert!(wait_for_frame(&mut client_a, Duration::from_secs(2)));

    // Use flock: create a workspace and write output into its pane.
...
fn cross_area_client_and_api_workspace_views_are_consistent() {
    ...
    client_handshake(&mut client, support::PROTOCOL_VERSION, 100, 30);
...
fn cross_area_two_clients_shared_view_and_single_detach_stability() {
    ...
    let mut client_a = UnixStream::connect(&client_socket).expect("client A should connect");
    client_handshake(&mut client_a, support::PROTOCOL_VERSION, 110, 30);
    let mut client_b = UnixStream::connect(&client_socket).expect("client B should connect");
    client_handshake(&mut client_b, support::PROTOCOL_VERSION, 100, 30);
...
fn cross_area_server_kill_then_restart_and_reconnect() {
    ...
    client_handshake(&mut reconnect_client, support::PROTOCOL_VERSION, 80, 24);
    assert!(
    ...
    cleanup_spawned_flock(server2, base);
}
