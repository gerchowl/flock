...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/flock-multi-client-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn server_log_path(config_home: &Path) -> PathBuf {
    ...
    config_home.join(app_dir).join("flock-server.log")
}
...
fn connect_raw_client(client_socket: &Path, cols: u16, rows: u16) -> UnixStream {
    ...
    client_handshake(&mut stream, 24, cols, rows).expect("handshake should succeed");
    stream
}
