...
fn client_handshake_succeeds() {
    ...
    assert_eq!(version, 19, "server should report protocol version 19");
    assert!(
        error.is_none(),
...
fn client_handshake_rejects_incompatible_version() {
    ...
    assert_eq!(version, 19, "server should report its version");
    assert!(
        error.is_some(),
...
fn client_handshake_clamps_small_terminal_size() {
    ...
    let (version, error) = client_handshake(&mut stream, 19, 0, 0)
...
fn client_handshake_with_host_theme_succeeds() {
    ...
    let (version, error) =
        support::client_handshake_with_theme(&mut stream, 19, 80, 24, &theme_bytes)
...
fn no_hello_client_closed_within_five_seconds() {
    ...
    let (version, error) = client_handshake(&mut good_stream, 19, 80, 24)
