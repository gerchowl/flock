...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_flock "$session" server stop >/dev/null 2>&1 || true
  done
...
run_flock() {
  ...
  env -u FLOCK_SOCKET_PATH \
    -u FLOCK_CLIENT_SOCKET_PATH \
    -u FLOCK_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$FLOCK_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/flock-dev' "$CONFIG_HOME"
  else
    printf '%s/flock-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/flock.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/flock-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/flock-dev/flock.sock | "$CONFIG_HOME"/flock-dev/sessions/*/flock.sock)
      ;;
    *)
...
echo "using flock: $FLOCK_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/flock-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/flock-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_flock "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
