...
Seeds a running flock server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  FLOCK_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/flock-dev/flock.sock.
  FLOCK_NAV_CWD          Workspace cwd for created panes. Defaults to the repo root.
  FLOCK_NAV_BIN          Flock binary to call. Defaults to cargo run from the repo.
USAGE
}
...
workspace_cwd="${FLOCK_NAV_CWD:-$repo_dir}"
config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
dev_socket="$config_home/flock-dev/flock.sock"
main_socket="$config_home/flock/flock.sock"
export FLOCK_SOCKET_PATH="${FLOCK_NAV_SOCKET_PATH:-$dev_socket}"

if [[ "$allow_main" != 1 && "$FLOCK_SOCKET_PATH" == "$main_socket" ]]; then
  echo "refusing to seed main flock session: $FLOCK_SOCKET_PATH" >&2
  echo "use FLOCK_NAV_SOCKET_PATH for a dev socket, or pass --allow-main intentionally" >&2
  exit 1
fi

if [[ ! -S "$FLOCK_SOCKET_PATH" ]]; then
  echo "flock socket not found: $FLOCK_SOCKET_PATH" >&2
  echo "start a dev server first, or set FLOCK_NAV_SOCKET_PATH" >&2
  exit 1
fi

cd "$repo_dir"

if [[ -n "${FLOCK_NAV_BIN:-}" ]]; then
  run() { "$FLOCK_NAV_BIN" "$@"; }
else
  run() { cargo run --quiet -- "$@"; }
fi
...
cat <<EOF
Seeded navigator demo data via $FLOCK_SOCKET_PATH
