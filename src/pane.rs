...
pub use self::{
    state::{CompletionPending, PaneState},
...
fn apply_pane_terminal_env(cmd: &mut CommandBuilder) {
    // Each pane is rendered by flock's own terminal layer, not the outer terminal
...
impl PaneRuntime {
    ...
    pub(crate) fn spawn_with_initial_history(
        ...
        cmd.cwd(cwd);
        cmd.env(crate::FLOCK_ENV_VAR, crate::FLOCK_ENV_VALUE);
