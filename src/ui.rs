...
pub(crate) use self::{
    ...
    sidebar::{
        ...
        normalized_workspace_scroll, nth_local_agent_entry_index, server_band_slot_at,
        servers_section_height, sidebar_menu_row_rect, sidebar_section_divider_rect,
        space_head_idx, workspace_list_entries, workspace_list_rect, workspace_list_scroll_metrics,
...
fn compute_view_internal(
    ...
    let (workspace_card_areas, remote_card_areas, space_header_areas) = if app.sidebar_collapsed {
...
mod tests {
    ...
    fn temp_git_repo(branch: &str) -> std::path::PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("unix time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flock-ui-test-{unique}"));
