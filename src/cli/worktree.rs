...
fn worktree_kill(args: &[String]) -> std::io::Result<i32> {
    ...
    let Some(workspace_id) = workspace_id else {
        eprintln!(
            "usage: flk worktree kill --workspace ID [--dry-run] [--force] [--keep-branch] [--keep-procs] [--json]"
        );
        return Ok(2);
    };
...
fn print_worktree_help() {
    ...
    eprintln!(
        "  flk worktree kill --workspace ID [--dry-run] [--force] [--keep-branch] [--keep-procs] [--json]"
    );
