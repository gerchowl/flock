...
impl App {
    ...
    pub(super) fn handle_worktree_kill(
        ...
        //
        // Asked of the resolved target rather than of a workspace's
        // membership, so a kill addressed by path (#396) sweeps the same way:
        // a checkout nobody has open is exactly where an orphan outlives
        // everything that could have reported it.
        let protected_pids = crate::worktree::processes::protected_pids(params.caller_pid);
        let standing =
            crate::worktree::processes::processes_in_checkout(&target.checkout, &protected_pids);
