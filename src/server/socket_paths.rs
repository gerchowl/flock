...
///
/// Contractual override behavior for auto-detect uses `FLOCK_SOCKET_PATH`.
/// This variable is kept as a fallback for callers that explicitly need a
/// client-only override when `FLOCK_SOCKET_PATH` is not set.
pub const CLIENT_SOCKET_PATH_ENV_VAR: &str = "FLOCK_CLIENT_SOCKET_PATH";
...
/// 2. If `FLOCK_SOCKET_PATH` is set, derive the client socket path from it by
///    inserting `-client` before `.sock` (e.g. `flock.sock` -> `flock-client.sock`).
///    This keeps JSON API and client socket overrides consistent.
/// 3. Otherwise, honor `FLOCK_CLIENT_SOCKET_PATH` (legacy/testing fallback).
...
pub(crate) fn prepare_socket_path(path: &Path) -> io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "flock server is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn client_socket_path_derived_from_api_socket_override() {
        let path = client_socket_path_from_overrides(Some("/tmp/test-flock.sock"), None);
        assert_eq!(path, PathBuf::from("/tmp/test-flock-client.sock"));
    }
    ...
    fn client_socket_path_respects_legacy_client_override_without_api_override() {
        let path = client_socket_path_from_overrides(None, Some("/tmp/test-flock-client.sock"));
        assert_eq!(path, PathBuf::from("/tmp/test-flock-client.sock"));
    }
    ...
    fn client_socket_path_defaults_to_config_dir() {
        ...
        assert_eq!(path, crate::config::config_dir().join("flock-client.sock"));
    }
