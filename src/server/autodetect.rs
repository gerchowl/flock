...
fn validate_running_server_compatibility() -> io::Result<()> {
    ...
    Err(io::Error::other(format!(
        "Flock was updated, but this session is still running the old server.\n\nserver: v{} protocol {}\nclient: v{} protocol {}\n\n{}",
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `FLOCK_SESSION`,
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/flock-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("FLOCK_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var("FLOCK_CLIENT_SOCKET_PATH");
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
