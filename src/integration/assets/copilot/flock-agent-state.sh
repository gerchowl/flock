...
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/flock-copilot-hook.XXXXXX")" || exit 0
...
FLOCK_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
...
try:
    ...
    if event_key == "sessionstart":
        ...
        # ownership until a real session exit so Flock can still track later turns.
