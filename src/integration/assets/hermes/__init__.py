"""Hermes plugin installed by Flock to report agent lifecycle state."""

# FLOCK_INTEGRATION_ID=hermes
...
_SOURCE = "flock:hermes"
_AGENT = "hermes"


def _base_params() -> tuple[str, str] | None:
    if os.environ.get("FLOCK_ENV") != "1":
        return None
    pane_id = os.environ.get("FLOCK_PANE_ID", "").strip()
    socket_path = os.environ.get("FLOCK_SOCKET_PATH", "").strip()
