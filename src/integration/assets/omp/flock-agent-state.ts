...
// FLOCK_INTEGRATION_ID=omp
// FLOCK_INTEGRATION_VERSION=2
// @ts-nocheck

import { createConnection } from "node:net";

const FLOCK_ENV = process.env.FLOCK_ENV;
const socketPath = process.env.FLOCK_SOCKET_PATH;
const paneId = process.env.FLOCK_PANE_ID;
const source = "flock:omp";

function enabled() {
  return FLOCK_ENV === "1" && !!socketPath && !!paneId;
}
...
const idleDebounceMs = parseDurationEnv("FLOCK_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("FLOCK_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("flock:blocked", (data) => {
    if (!data?.active) {
