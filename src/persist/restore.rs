...
fn restore_tab(
    ...
    for id in &pane_ids {
        ...
        let saved_spawn_depth = saved_pane.map_or(0, |p| p.spawn_depth);
        let saved_spawned_by = saved_pane.and_then(|p| p.spawned_by.clone());
        ...
        if let Some(plan) = pending_native_agent_restore {
            ...
            terminal.spawn_depth = saved_spawn_depth;
            terminal.spawned_by = saved_spawned_by.clone();
...
mod tests {
    ...
    fn restored_worktree_space_membership_drops_missing_checkout() {
        let missing =
            std::env::temp_dir().join(format!("flock-missing-worktree-{}", std::process::id()));
    ...
    fn restore_rehydrates_agent_session_metadata() {
        ...
        assert_eq!(preserved.source, "flock:hermes");
    ...
    async fn restore_carries_persisted_agent_session_metadata() {
        ...
        assert_eq!(session.source, "flock:opencode");
