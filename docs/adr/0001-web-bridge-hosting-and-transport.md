...
- Decision owner: human; advised by a parallel review panel (flock-protocol,
  fleet-topology, security, Rust-packaging) + a post-implementation review round.

This is flock's first ADR; it seeds `docs/adr/` with sequential numbering.
...
- A headless `flock server` participates **fully** in fleet gossip with no TUI
...
3. **Transport — PTY subprocess for v1.** The bridge spawns `flock client` in a
   ...
   `flock-client.sock` bincode bridge or a `--stdio` no-PTY mode is deferred
...
5. **Packaging — in-tree, behind a cargo feature.** A `flock web` subcommand
...
6. **Security boundary — loopback bind + `tailscale serve`, with self-defence.**
   ...
   clean flock environment (inherited `FLOCK_*` stripped) so it isn't killed by
...
- **Keeping the bridge in g-fleet** — rejected; it is product, entirely coupled
  to flock's `terminal-ansi` output, and belongs versioned with flock. g-fleet
  keeps only `enable + tailscale serve` (#149).

## Consequences

- Any host can opt into `flock web`; g-fleet just picks which and fronts it.
...
- A persistent `flock server` daemon must actually run on the host for the
