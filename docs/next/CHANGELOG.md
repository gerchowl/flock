...
- A server's view of the fleet no longer depends on which server you happen to be attached to. Peers known only through relay (two hops out) were gossiped into the local cache and forwarded onward, but never rendered locally, so a machine could see strictly less of the fleet than the machine next to it. They now render as ordinary rows in the servers band, the spaces list, the navigator, and the attention cycle, losing to a locally-polled row about the same host. A leg that passes a carried snapshot on also unions in what it can see — its own polled peers, its relayed entries, and itself — rather than forwarding the first hub's view verbatim, so the fleet no longer shrinks as the chain gets longer. (#80)
