  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      branch: Type.Optional(
        Type.String({
          description:
            "Branch name for the new worktree. If omitted, Flock generates a worktree/* branch.",
        }),
      ),
      base: Type.Optional(
        Type.String({
      ...
      label: Type.Optional(
        Type.String({
          description: "Workspace label for the new Flock worktree workspace.",
        }),
      ),
      closeOldPane: Type.Optional(
        Type.Boolean({
          description: "Close the old Flock pane after the old pi process exits. Defaults to true.",
        }),
      ),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      return startFlockWorktree(pi, ctx, signal, {
  ...
  pi.registerCommand("flock-worktree-start", {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        const result = await startFlockWorktree(pi, ctx, undefined, parsed);
...
async function startFlockWorktree(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
  signal: AbortSignal | undefined,
  options: StartOptions,
) {
  if (process.env.FLOCK_ENV !== "1") {
    throw new Error("flock_start_worktree must run inside a Flock-managed pane");
  }

  const oldPaneId = process.env.FLOCK_PANE_ID;
  if (options.closeOldPane && !oldPaneId) {
    throw new Error("FLOCK_PANE_ID is missing; cannot close the old Flock pane safely");
  }
  ...
  ctx.ui.setStatus("flock-worktree", "creating worktree");

  let newSessionFile: string | undefined;
  try {
    const created = await createFlockWorktree(pi, signal, sourceCheckout, options);
    ...
    if (created.workspaceId) {
      await flock(pi, ["workspace", "focus", created.workspaceId], signal, sourceCheckout, 10_000);
    }
    ...
    ctx.ui.setStatus("flock-worktree", undefined);
    ctx.ui.notify(`Started pi in Flock worktree: ${worktreePath}`, "info");
    ctx.shutdown();
      ...
      content: [
        {
          type: "text" as const,
          text:
            `Started replacement pi in Flock worktree: ${worktreePath}\n` +
            `Workspace: ${created.workspaceId ?? "unknown"}\n` +
            `Pane: ${created.rootPaneId}\n` +
            `Branch: ${created.branch ?? "generated by Flock"}\n\n` +
...
async function createFlockWorktree(
  pi: ExtensionAPI,
  signal: AbortSignal | undefined,
  sourceCheckout: string,
  options: StartOptions,
  ...
  const json = await flockJson(pi, args, signal, sourceCheckout, 120_000);
  const worktreePath = json.result?.worktree?.path;
  const rootPaneId = json.result?.root_pane?.pane_id;
  if (!worktreePath || !rootPaneId) {
    throw new Error("Flock worktree create response did not include worktree.path and root_pane.pane_id");
  }
...
async function runInNewPane(
  ...
  await flock(pi, ["pane", "run", paneId, command], signal, undefined, 10_000);
}
...
async function scheduleOldPaneCleanup(
  ...
  const cleanup = [
    ...
    "rm -f -- \"$old_session\"",
    "flock pane close \"$old_pane\" >/dev/null 2>&1 || true",
  ].join("; ");
...
async function flockJson(
  pi: ExtensionAPI,
  args: string[],
  signal: AbortSignal | undefined,
  cwd: string | undefined,
  timeout: number,
): Promise<FlockWorktreeResult> {
  const result = await flock(pi, args, signal, cwd, timeout);
  const raw = result.stdout.trim() || result.stderr.trim();
  let json: FlockWorktreeResult;
  try {
    json = JSON.parse(raw) as FlockWorktreeResult;
  } catch {
    throw new Error(`Flock returned non-JSON output for ${args.join(" ")}: ${raw}`);
  }
  if (json.error) {
    throw new Error(`${json.error.code ?? "flock_error"}: ${json.error.message ?? "unknown Flock error"}`);
  }
  return json;
}
...
async function flock(
  ...
  const result = await pi.exec("flock", args, { cwd, signal, timeout });
  if (result.code !== 0) {
    throw new Error(`flock ${args.join(" ")} failed: ${result.stderr || result.stdout}`);
  }
  return result;
}
